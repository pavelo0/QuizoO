-- Демонстрационные русскоязычные модули (флешкарты + квиз) для локальной проверки.
-- Владелец demo-данных: percheck298@gmail.com (роль USER).
-- Если пользователь отсутствует, он создается автоматически.
-- Дополнительно очищаются все модули у владельцев с ролью ADMIN.
...
BEGIN
  ...
  -- У администраторов не должно быть учебных модулей.
  DELETE FROM "modules" m
  USING "users" u
  WHERE m."userId" = u."id" AND u."role" = 'ADMIN';

  -- На случай повторного прогона удаляем предыдущие demo-модули по фиксированным id.
  DELETE FROM "modules"
